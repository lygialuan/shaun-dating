...
namespace Packages\ShaunSocial\Vibb\Http\Controllers\Admin;
...
class SongController extends Controller
{
    ...
    {
        $this->middleware('has.permission:admin.vibb.song_manage');
    }
    ...
    public function index(Request $request)
    {
        ...
        $title = __('Vibb Songs');
        $builder = VibbSong::orderBy('id', 'desc');
        ...
        return view('shaun_vibb::admin.song.index', compact('breadcrumbs', 'title', 'songs', 'statusArray', 'status', 'name', 'defaultArray' ,'default'));
    }
    ...
    public function create($id = null)
    {
        if (empty($id)) {
            ...
            $song = VibbSong::findOrFail($id);
        }

        $title = empty($id) ? __('Create') : __('Edit');

        return view('shaun_vibb::admin.song.create', compact('title', 'song'));
    }
    ...
    public function store(Request $request)
    {
        ...
        $rules = [
            ...
            'file' => ['required', new FileValidation(config('shaun_vibb.song_validate'))]
        ];
        ...
        $data = $request->only(['is_active', 'name']);

        if ($request->id) {
            $song = VibbSong::findOrFail($request->id);
            $song->update($data);
        } else {
            $song = VibbSong::create($data);
        }
