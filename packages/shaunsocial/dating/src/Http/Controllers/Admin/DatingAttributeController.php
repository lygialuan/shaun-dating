...
class DatingAttributeController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $attributes = DatingAttribute::getByCategoryId(0);

        return view( 'shaun_dating::admin.attribute.index', compact('breadcrumbs', 'title', 'attributes'));
    }
    ...
    public function create(Request $request)
    {
        ...
        if (!$id) {
            $otherAttributes = DatingAttribute::get();
        }

        if (empty($id)) {
            $attribute = new DatingAttribute();
            $attribute->is_active = true;
        } else {
            $attribute = DatingAttribute::findByField('id', $id);
        }

        $title = empty($id) ? __('Create') : __('Edit');

        return view( 'shaun_dating::admin.attribute.create', compact('title', 'attribute', 'otherAttributes'));
    }
    ...
    public function store(Request $request)
    {
            ...
            [
                ...
                'clone_from' => [
                    ...
                    function ($_, $cloneFrom, $fail) {
                        if ($cloneFrom) {
                            $attribute = DatingAttribute::findByField('id', $cloneFrom);
                            if (!$attribute) {
        ...
        $oldAttribute = DatingAttribute::findByField('id', $request->clone_from);

        if ($request->id) {
            $category = DatingAttribute::findOrFail($request->id);
            $category->update($data);
        } else {
            if ($request->clone_from) {
                $category = DatingAttribute::create([
                    'name' => $request->name,
                    'icon_file_id' => null,
                ...
                $values = DatingAttribute::getAttributeValues($oldAttribute->id);
                foreach ($values as $value) {
                    $value->cloneValue($category->id);
                } 
            }
            else {
                $category = DatingAttribute::create($data);
            }
        }
    ...
    {
        $attribute = DatingAttribute::findOrFail($id);

        $attribute->delete();

        return redirect()->route('admin.dating.attribute', $attribute->category_id)->with([
    ...
    public function store_order(Request $request)
    {
        ...
        foreach ($orders as $order => $id) {
            $tag = DatingAttribute::findByField('id', $id);
            if (! $tag) {
                continue;
            }
            $tag->update([
