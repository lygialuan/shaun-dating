 ...
 */
 ...
 namespace Packages\ShaunSocial\Core\Library;
...
class OpenIDConnectClient
{
    ...
    public $scopeOpenid = true;
    public $codeVerifierRandomCount = 64;
    public $needCheckAccessToken = true;
    ...
    public function authenticate() {
        ...
        if (isset($_REQUEST['code'])) {
            ...
            if (isset($token_json->error)) {
                ...
                $message = $token_json->error;
				if (! is_string($message)) {
					$message = 'Error';
				}
                throw new OpenIDConnectClientException('Got response: ' . $message);
            }
            $this->accessToken = $token_json->access_token;
            
            if (! $this->needCheckAccessToken) {
                return true;
            }
            ...
            if ($_REQUEST['state'] !== $this->getState()) {
                throw new OpenIDConnectClientException('Unable to determine state '.$this->getState().' - '. $_REQUEST['state']);
            }
    ...
    private function requestAuthorization() {
        ...
        if ($this->scopeOpenid == false && !empty($auth_params['scope'])) {
            $auth_params['scope'] = trim(str_replace('openid', '', $auth_params['scope']));
        }
        ...
        if (!empty($codeChallengeMethod) && in_array($codeChallengeMethod, $this->getProviderConfigValue('code_challenge_methods_supported', []), true)) {
            $codeVerifier = bin2hex(random_bytes($this->codeVerifierRandomCount));
        ...
        //$this->commitSession();
        return $this->redirect($auth_endpoint);
    }
