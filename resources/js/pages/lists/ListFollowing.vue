<template>
    <div class="flex items-center justify-between gap-3 mb-5">
        <div class="flex items-center gap-base-2">
            ...
            <h2 class="text-xl font-bold">{{ $t('Following') }}</h2>
        </div>
    </div>
    <div class="tabs-list flex gap-base-2 mb-4">
        <button @click="filterFollowingUsersList('user')" class="tabs-list-item p-base-2 rounded-base-lg" :class="followingType === 'user' ? 'active bg-primary-color text-white dark:bg-dark:bg-slate-800' : 'bg-web-wash dark:bg-dark-web-wash'">{{ $t('Users') }}</button>
        <button @click="filterFollowingUsersList('page')" class="tabs-list-item p-base-2 rounded-base-lg" :class="followingType === 'page' ? 'active bg-primary-color text-white dark:bg-dark:bg-slate-800' : 'bg-web-wash dark:bg-dark-web-wash'">{{ $t('Pages') }}</button>
    </div>
    <Error v-if="error">{{error}}</Error>
    <UsersList :loading="loadingUsers" :users-list="followingUsersList" :has-load-more="loadmoreStatus" @load-more="getFollowingUsersList(followingType, page)">
        <template #empty>
            <div class="text-center">
                {{ followingType === 'user' ? $t('No Users') : $t('No Pages') }}
            </div>
		</template>
    </UsersList>
</template>
