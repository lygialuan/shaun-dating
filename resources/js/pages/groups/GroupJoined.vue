...
export default {
    ...
    mounted() {
        this.getJoinedGroupsList(this.currentPage)
    },
    unmounted(){
    ...
    methods: {
        ...mapActions(useGroupStore, ['getJoinedGroupsList', 'unsetGroupsList']),
        loadmoreSuggestGroups($state) {
			this.getJoinedGroupsList(++this.currentPage).then((response) => {
